...
```{r}
sep_full <- left_join(sep_old, sep_new, by = c("title", "id", "department"))
...
top_incr <- sep_full %>% 
  ...
  select("id", "department", "title", "difference") %>% 
  head(5)
...
```{r}
full_join(top_incr, top_decr, by = c("title", "difference")) %>% 
  ggplot(aes(x = title, y = difference)) + geom_col() + ylim(-25,25) + theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
